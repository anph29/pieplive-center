...
class PThread(QtCore.QThread):
    ...
    def timelap(self):
        ...
        if ret:
            ...
            pix = convertToQtFormat.scaled(
                self.vH, self.vW, QtCore.Qt.KeepAspectRatio)
...
class QtCameta(QtWidgets.QWidget):
    vH = 135
    vW = 240
    RTSP = 'RTSP'
    M3U8 = 'M3U8'
    ...
    def initUI(self):
        if self.mtype == self.RTSP:
            self.initCv2Cam()
        else:
            self.initSimpleCam()

    def initSimpleCam(self):
        self.itemCam = uic.loadUi("./src/ui/item_cam.ui", self)
        self.video = QVideoWidget()
        self.player = QMediaPlayer()
        self.player.setVideoOutput(self.video)
        self.player.setMedia(QMediaContent(
            QtCore.QUrl.fromUserInput(self.url)))
        # to start at the beginning of the video every time
        self.player.setPosition(0)
        self.player.setVolume(0)
        self.itemCam.video_holder.addWidget(self.video)
        self.itemCam.lbl_name.setText(self.name)
        self.video.show()
        self.player.play()
