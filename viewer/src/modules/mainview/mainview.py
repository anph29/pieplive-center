...
class MainView(tk.Frame):
    ...
    def setStyle(self):
        ...
        style.theme_create("TabStyle", parent="alt", settings={
            ...
            # NOTEBOOK
            "TNotebook": {  # big zone
                "configure": {"background": "#fff", "tabmargins": [0, 0, 0, 0], "relief": tk.FLAT}
            },
    ...
    def packRightToolbar(self):
        ...
        #
        imgCheck = ImageTk.PhotoImage(Image.open(
            helper._ICONS_PATH+'close-pink-s.png'))
        lblCommandClear = tk.Label(
            rightToolbar, bd=1, image=imgCheck, bg="#fff")
        ...
        #
        imgCheck = ImageTk.PhotoImage(Image.open(
            helper._ICONS_PATH+'check-green-s.png'))
        lblCommandCheck = tk.Label(
            rightToolbar, bd=1, image=imgCheck, bg="#fff")
        ...
        cbxQ170 = LabeledCombobox(rightToolbar, cbxData, callback=self.onSelectBussiness,
                                  selected=store.getCurrentActiveBusiness(), bd=1, relief=tk.FLAT)
    ...
    def onNewResource(self, evt):
        if messagebox.askyesno("PiepMe", "Are you sure to renew your bussiness resource?"):
            ...
            if len(lsL500) > 0:
                # presenter
                presenter = list(
                    filter(lambda l500: l500['LN508'] == 1, lsL500))
