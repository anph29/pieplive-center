...
class PopupAddSchedule(object):
    ...
    def showChangeRuntimeUI(self):
        ...
        self.popupRuntime = tk_helper.makePiepMePopup(
            "Change runtime", w=300, h=120, padx=0, pady=0
        )
        # var
        self.HH = tk.StringVar()
        self.HH.trace("w", self.limitHH)
        ...
        btnCancel = tk.Button(
            ...
            bd=2,
            relief=tk.RAISED,
            command=self.popupRuntime.destroy,
        )
        btnCancel.configure(width=7)
        btnCancel.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)
        btnOk = tk.Button(
        ...
        self.parent.calcRuntime(
            {
                ...
                "timepoint": helper.convertHMSNoToSec(
                    {
                        "h": int(self.HH.get()),
                        "m": int(self.MM.get()),
                        "s": int(self.SS.get()),
