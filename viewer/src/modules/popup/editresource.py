...
class PopupEditResource(PopupAddResource):
    ...
    def initGUI(self, data):
        ...
        if None is not self.popup:
            self.popup.destroy()
        self.popup = tk_helper.makePiepMePopup('Edit Media', w=450, h=200, padx=0, pady=0)
        self.initTabFileUI()
    ...
    def onSave(self):
        if self.mtype == 'VIDEO':
            self.duration = helper.convertHMSNoToSec({'h': int(self.hh.get()), 'm': int(self.mm.get()), 's': int(self.ss.get())}) 
