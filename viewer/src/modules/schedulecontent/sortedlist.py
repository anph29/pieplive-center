...
class SortedList(ScheduleDDList):
    ...
    def duplicateSchedule(self, evt):
        filtered = list(filter(lambda x: x.checked.get() and bool(x.path), self._LS_SCHEDULE_UI))
        if len(filtered) > 0:        
            if messagebox.askyesno("PiepMe", "Are you sure duplicate all selected schedule?"):  
                for sch in filtered:
